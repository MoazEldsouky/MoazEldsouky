# Hi 👋, I'm Moaz Eldsouky

## AI Engineer | LLM, RAG & Multimodal AI

[![LinkedIn](https://img.shields.io/badge/-LinkedIn-0077B5?style=flat&logo=linkedin&logoColor=white)](https://www.linkedin.com/in/moaz-eldesouky-762288251/)
[![Email](https://img.shields.io/badge/-Email-D14836?style=flat&logo=gmail&logoColor=white)](mailto:moazeldsoky8@gmail.com)
[![Kaggle](https://img.shields.io/badge/-Kaggle-20BEFF?style=flat&logo=kaggle&logoColor=white)](https://kaggle.com/moazeldsokyx)
<a href="https://moazeldsouky.github.io/Portfolio"><img src="https://img.shields.io/badge/-Portfolio-000000?style=for-the-badge&logo=github&logoColor=white" height="28" alt="Portfolio" align="center" /></a>

---

## 🚀 About Me

AI Engineer with 1+ years of experience building production LLM and RAG systems for enterprise, government, and healthcare organizations. I specialize in fully offline, on-premises deployment of LLMs, VLMs, OCR, embedding, and reranking models. My deep focus is on Arabic-language document AI, including VLM OCR fine-tuning for handwritten legal text where open-source baselines consistently fail.

- 💼 **Current Role**: Artificial Intelligence Engineer at **Skygate Information Technology** (May 2026 - Present)
- 🔬 **Expertise**: Large-scale RAG architecture, LLM/VLM fine-tuning (LoRA, QLoRA), hybrid retrieval, and multimodal document AI
- 📊 **Freelance Success**: Delivered **25+ end-to-end AI/ML solutions** with 100% client satisfaction
- 🎓 **Education**: B.Sc. in Artificial Intelligence from Kafrelsheikh University (Graduated Jul 2025, Grade: Very Good)
- 🌍 **Languages**: Arabic (Native), English (Professional Working Proficiency)
- 📍 **Location**: Cairo, Egypt (Open to Remote & Relocation)

---

## 💼 Professional Experience

### Artificial Intelligence Engineer @ Skygate Information Technology (an E-VAS TELECOM company)
**May 2026 - Present | Full-time | Cairo, Egypt**

- Built an enterprise-scale RAG platform that ingests and indexes millions of documents into a Qdrant vector database.
- Architected fully offline, on-premises AI systems satisfying strict privacy, security, and regulatory compliance requirements with zero external API dependencies.
- Designed a vault-based multi-tenant architecture with role-based access control (RBAC) to isolate knowledge bases across departments like Finance, HR, and Legal.
- Created a multimodal document processing pipeline handling PDF, TXT, Markdown, Microsoft Word, Excel, PowerPoint, images, audio, and video formats.
- Optimized large-scale inference pipelines for high throughput on NVIDIA H200 GPU servers using vLLM.

### AI Software Engineer @ Volaris LLC
**September 2025 - April 2026 | Hybrid**

- Delivered a production RAG system for medical specialists over ASCO, ESMO, NCCN, NICE, and WHO guidelines.
- Achieved evidence-based recommendations in under 4 seconds using medical-aware embedding models and FAISS vector search.
- Automated an HBV Clinical Assessment Engine evaluating 15+ clinical parameters to replace a multi-hour manual review workflow.
- Developed a context-aware clinical chat system with persistent session memory and full source attribution.
- Shipped AI services as containerized FastAPI REST APIs on Azure with CI/CD pipelines and managed PostgreSQL infrastructure.

### Freelance AI Engineer @ Mostaql Platform
**March 2024 - Present | Remote**

- Delivered 25+ end-to-end AI/ML solutions for SME and startup clients across healthcare, e-commerce, and finance.
- Fine-tuned LLMs with LoRA and QLoRA for domain-specific QA, code repair, and text classification achieving 95%+ accuracy.
- Built LLM-powered autonomous assistants on OpenAI, Claude, and Groq with multi-step LangGraph agentic workflows.
- Deployed RAG pipelines on FAISS, Chroma, and Pinecone to measurably improve retrieval grounding over vanilla LLM responses.

---

## 🛠️ Technical Stack

### **Core Technologies & Frameworks**
- **Libraries**: PyTorch, Hugging Face Transformers, LangChain, LangGraph, CrewAI, Scikit-learn, spaCy, NLTK
- **Models API**: OpenAI, Claude (Anthropic), Gemini, Groq

### **AI/ML Specializations**
- **LLMs & Generative AI**: Fine-tuning (LoRA, QLoRA), air-gapped deployment, agentic AI, autonomous agents
- **Document AI & Multimodal**: Vision Language Models (VLMs), OCR pipelines, Arabic handwriting recognition, layout and table extraction
- **RAG & Retrieval**: Hybrid retrieval (BM25 + dense), reranking models, chunking strategies, high-volume ingestion
- **Vector Databases**: Qdrant, FAISS, Chroma, Pinecone

### **Infrastructure & MLOps**
- **Hardware & Serving**: On-premises AI infrastructure, NVIDIA GPUs servers, vLLM, GPU inference optimization
- **Deployment**: FastAPI, Docker, CI/CD, Azure, AWS, PostgreSQL
- **Security**: Role-based access control (RBAC), tenant/vault isolation, regulatory compliance

---

## 🏆 Featured Projects

### 📜 **VLM OCR Fine-Tuning for Arabic Legal Documents**
- Fine-tuned a Vision Language Model OCR system for Arabic legal documents and contracts containing mixed handwritten and printed text.
- Raised recognition accuracy on handwritten Arabic within the target document domain where leading state-of-the-art baselines consistently failed.
- Assembled a domain-specific training corpus via public datasets, targeted web scraping, and synthetic data generation.

### 🤖 **Agentic Medical RAG Chatbot**
- Engineered a multi-tool AI agent using LangGraph and LangChain that routes queries between an internal RAG system and external web search.
- Added an intelligent consultation-booking subsystem to validate inputs, collect patient details, and auto-handle optional fields.

### 🇸🇦 **AraBERTv02 Fine-Tuning for Arabic Sentiment Analysis**
- Curated and labeled 1,000 Arabic restaurant reviews.
- Fine-tuned AraBERTv02 for binary sentiment classification, achieving 99% accuracy on the test set.
- Published the model as a live public demo on Hugging Face Spaces.

### 🔍 **DeBERTa Fine-Tuning for Machine-Generated Text Detection**
- Trained DeBERTa with custom word embeddings to detect machine-generated social media text.
- Achieved 97%+ accuracy, directly applicable to content moderation and trust & safety pipelines.

---

## 🎯 Professional Highlights

- ✅ **Production RAG & LLMs**: Architected enterprise platforms processing millions of documents with hybrid retrieval and strict access control.
- ✅ **Proven Freelance Record**: 25+ freelance AI projects delivered with 100% client satisfaction.
- ✅ **Advanced Document AI**: Delivered Arabic VLM OCR fine-tuning in production environments.
- ✅ **Air-Gapped Infrastructure**: Deployed complete AI stacks locally on NVIDIA H200 servers with zero external API dependencies.

---

## 📫 Let's Connect

I'm passionate about building intelligent AI systems that solve real-world problems. Whether you're interested in:

- 🤝 **Collaborating** on cutting-edge AI projects
- 💡 **Discussing** LLMs, RAG systems, or AI agents
- 🚀 **Building** production-ready ML solutions
- 📚 **Sharing** knowledge about AI engineering

**Feel free to reach out!**

<div align="center">

<a href="https://moazeldsouky.github.io/Portfolio">
  <img src="https://img.shields.io/badge/🌐_Visit_My_Interactive_Portfolio-000000?style=for-the-badge&logo=github&logoColor=white" height="42" alt="Portfolio" />
</a>

<br /><br />

[![LinkedIn](https://img.shields.io/badge/-Connect%20on%20LinkedIn-0077B5?style=for-the-badge&logo=linkedin&logoColor=white)](https://www.linkedin.com/in/moaz-eldesouky-762288251/)
[![Email](https://img.shields.io/badge/-Email%20Me-D14836?style=for-the-badge&logo=gmail&logoColor=white)](mailto:moazeldsoky8@gmail.com)
[![Kaggle](https://img.shields.io/badge/-Kaggle-20BEFF?style=for-the-badge&logo=kaggle&logoColor=white)](https://kaggle.com/moazeldsokyx)
[![WhatsApp](https://img.shields.io/badge/-WhatsApp-25D366?style=for-the-badge&logo=whatsapp&logoColor=white)](https://wa.me/+201096448317)

</div>

---

<div align="center">
  <i>⭐ If you find my work valuable, consider starring my repositories!</i>
  <br>
  <i>💼 Open to AI Engineering opportunities and collaborations</i>
</div>
